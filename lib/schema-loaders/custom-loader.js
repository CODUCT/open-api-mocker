...
module.exports = class CustomSchemaLoader extends EventEmitter {
    load(schemaPath) {
        ...
        const servers = customSchema.servers;
        const routes = Object.entries(customSchema.paths);

        for (const [route, methods] of routes) {
            for (const [method, methodConfig] of Object.entries(methods)) {
                const routes = servers.map(server => new URL(path.join(server.url, route)).pathname);
                const externalJSPath = methodConfig["x-external-processor"]?.path;
                if (externalJSPath) {
                    ExternalJSContainer.getInstance().paths.push({ routes, method, path: externalJSPath });
                }
            }
        }
