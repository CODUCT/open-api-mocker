...
module.exports = class CustomSchemaLoader extends EventEmitter {
    load(schemaPath) {
        ...
        const serverRoutes = customSchema.servers;
        ...
        for (const server of serverRoutes) {
            const externalJSPath = server["x-external-processor"]?.path;
            if (externalJSPath) {
                console.log("External Processor loaded for: " + server.url);
                const url = new URL(server.url);
                const path = url.pathname;
                ExternalJSContainer.getInstance().topLevelPaths.push({ server: path, path: externalJSPath });
            }
        }

        console.log(ExternalJSContainer.getInstance().paths);
        console.log(ExternalJSContainer.getInstance().topLevelPaths);
