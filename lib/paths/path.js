...
class Path {
	...
	getResponse(preferredStatusCode, preferredExampleName) {
		...
		const validationPackage = {
			...
			requestBody: this.requestBodyData,
			requestHeaders: this.requestHeaders
		}
		const { response, isIntercepted, responseStatusCode, requestHeaders } = externalJSHandler.execute(validationPackage);

		let body = null;

		if (isIntercepted && response) {
			body = response;
		}
		else if (schema) {
			body = ResponseGenerator.generate(schema, preferredExampleName)
		}

		const returnObject = {
			statusCode: responseStatusCode ?? Number(statusCode),
			headers: requestHeaders ?? (headers && this.generateResponseHeaders(headers)),
			body: body,
			responseMimeType
		}
