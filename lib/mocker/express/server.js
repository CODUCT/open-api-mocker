...
const externalJSHandler = require('../../middlewares/external-js-handler');
...
class Server {
	...
	_notFoundHandler(req, res) {
		const defaultResponseStatusCode = 500;
		try {
			const requestedURI = req.originalUrl;
			const topLevelPaths = ExternalJSContainer.getInstance().topLevelPaths;

			for (const handler of topLevelPaths) {
				if (requestedURI.startsWith(handler.server)) {
					const { response, isIntercepted, responseStatusCode } = externalJSHandler.executeToplevel(req);

					if (isIntercepted) {
						let status = defaultResponseStatusCode;
						response
							.then((handlerResponse) => {
								status = handlerResponse.status;
								handlerResponse.json()
									.then((handlerResponseJson) => {
										if (!handlerResponseJson) throw new Error("No response body");
										this.sendResponse(req, res, handlerResponseJson, status);
									})
									.catch((err) => {
										this.sendResponse(req, res, {}, status);
									});
							})
							.catch((err) => {
								this.sendResponse(req, res, {}, status);
			...
			return this.sendResponse(req, res, {
				message: `Path not found: ${requestedURI}`,
				paths: validPaths
			}, 400);
		}
		catch (err) {
			this.sendResponse(req, res, {}, defaultResponseStatusCode);
		}
	}
